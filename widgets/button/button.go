...
func (btn *Button) CreateElement(ctx *goui.Context, parent goui.Element) (element goui.Element, err error) {
	...
	handle, err := ctx.App().OS().NewButton(parentHandle, btn.Label)
	if err != nil {
		err = errortrace.ErrorfStack("Create Button failed: %w", err)
		return
	}
	ctx.App().OS().Control_SetEnabled(handle, !btn.Disabled)
	...
	element = &buttonElement{
		goui.ControlElementHelper{
			...
			Handle:      handle,
			DestroyFunc: func(ctx *goui.Context, handle native.Handle) error { return ctx.App().OS().Window_Destroy(handle) },
		},
	}
	ctx.App().OS().Button_SetOnClickListener(handle, func() {
		if btn.OnClick != nil {
			btn.OnClick(ctx)
		}
	})
...
func (e *buttonElement) SetWidget(ctx *goui.Context, widget goui.Component) (err error) {
	...
	if oldBtn != nil {
		if oldBtn.Label != newBtn.Label {
			if err = ctx.App().OS().Button_SetLabel(e.Handle, newBtn.Label); err != nil {
				return
			}
		}
		if oldBtn.Disabled != newBtn.Disabled {
			if err = ctx.App().OS().Control_SetEnabled(e.Handle, !newBtn.Disabled); err != nil {
				return
			}
		}
	} else {
	...
	if newBtn.OnClick != nil {
		ctx.App().OS().Button_SetOnClickListener(e.Handle, func() { newBtn.OnClick(ctx) })
	} else {
		ctx.App().OS().Button_SetOnClickListener(e.Handle, nil)
	}
...
func (l *buttonLayouter) Layout(ctx *goui.Context, constraints metrics.Constraints) (size metrics.Size, err error) {
	...
	intrinsicSize, err := ctx.App().OS().Button_MinimumSize(elem.Handle, widget.Label)
	if err != nil {
		return
	}
...
func (l *buttonLayouter) PositionAt(ctx *goui.Context, pt metrics.Point) (err error) {
	return ctx.App().OS().Control_SetDimensions(l.Element().(*buttonElement).Handle, metrics.NewRect(pt, l.layoutSize))
}
