...
func (btn *Label) CreateElement(ctx *goui.Context, parent goui.Element) (element goui.Element, err error) {
	...
	handle, err := ctx.App().OS().NewLabel(parentHandle, btn.Text)
	if err != nil {
		err = errortrace.ErrorfStack("Create Label failed: %w", err)
		return
	}
...
func (e *labelElement) SetWidget(ctx *goui.Context, widget goui.Component) (err error) {
	...
	if oldLabel != nil {
		if oldLabel.Text != newLabel.Text {
			if err = ctx.App().OS().Label_SetText(e.Handle, newLabel.Text); err != nil {
				return
			}
		}
		if oldLabel.BackgroundColor != newLabel.BackgroundColor &&
			...
			if err = ctx.App().OS().Label_SetBackgroundColor(e.Handle, newLabel.BackgroundColor); err != nil {
				return
			}
		}
		if oldLabel.Multiline != newLabel.Multiline {
			if err = ctx.App().OS().Label_SetMultiline(e.Handle, newLabel.Multiline); err != nil {
				return
			}
		}
		if oldLabel.TextAlignment != newLabel.TextAlignment {
			if err = ctx.App().OS().Label_SetTextAlignment(e.Handle, native.TextAlignment(newLabel.TextAlignment)); err != nil {
				return
			}
		}
		return
	...
	return ctx.App().OS().Label_SetTextAlignment(e.Handle, native.TextAlignment(newLabel.TextAlignment))
}
...
func (l *labelLayouter) Layout(ctx *goui.Context, constraints metrics.Constraints) (size metrics.Size, err error) {
	...
	intrinsicSize, err := ctx.App().OS().Control_TextDrawingSize(elem.Handle, widget.Text, widget.Multiline, constraints.MaxWidth)
	if err != nil {
		return
	}
...
func (l *labelLayouter) PositionAt(ctx *goui.Context, pt metrics.Point) (err error) {
	return ctx.App().OS().Control_SetDimensions(l.Element().(*labelElement).Handle, metrics.NewRect(pt, l.layoutSize))
}
